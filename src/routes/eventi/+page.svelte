<script>
	...
	async function caricaEventi() {
		// Per ora dati mock, poi collegheremo il database
		eventi = [
			{
				id: 1,
	...
	function aggiungiEvento() {
		...
		const nuovoId = Math.max(0, ...eventi.map(e => e.id)) + 1;
		eventi = [...eventi, { ...nuovoEvento, id: nuovoId }];
	...
	function eliminaEvento(id) {
		if (confirm('Sei sicuro di voler eliminare questo evento?')) {
			eventi = eventi.filter(e => e.id !== id);
		}
	}
</script>
	...
	<div class="max-w-6xl mx-auto px-4">
		...
		<div class="bg-white rounded-lg shadow">
			<div class="px-6 py-4 border-b">
				<h2 class="text-xl font-semibold">Eventi Programmati ({eventi.length})</h2>
			</div>
						...
						<div class="p-6 hover:bg-gray-50 transition">
							<div class="flex items-center justify-between">
								...
								<div class="flex space-x-2">
									...
									<button 
										on:click={() => eliminaEvento(evento.id)}
